...
class State(object):
    ...
    def parse(self, msg):
        ...
        2、遍历字典：
        （1）获取字典的键、值
        （2）对类属性进行调整（self._key=value）
        3、根据（游戏阶段，消息类型）做出处理（主要输出信息）
        :param msg: 消息字典
        """
        ...
        try:
            self.__parse_func[(self._stage, self._type)]()  # 输出处理信息
            self._stage = None
            self._type = None
        except KeyError:
    ...
    def notify_begin(self):
        ...
        """
        # TODO: 选手可自行做出其他处理
        information.person.update_card(self._handCards)  # 获取手牌
        information.person.set_pos(self._myPos)  # 获取位置，并计算牌力
        information.record.init_card_recorder(self._handCards)  # 记牌器初始化
        strategy.getpos(self._myPos)
    ...
    def notify_play(self):
        ...
        information.record.update_card_recorder(self._curPos, self._curAction)  # 更新记牌器
    ...
    def notify_tribute(self):
        ...
        for tribute_result in self._result:
            ...
            information.record.update_card_tribute(tribute_pos, receive_tribute_pos, card)
    ...
    def notify_back(self):
        ...
        for back_result in self._result:
            ...
            information.record.update_card_back(back_pos, receive_back_pos, card)
    ...
    def notify_episode_over(self):
        ...
        # for i in range(4):
        #     if self._order[i] == information.person.my_pos:
        #         with open(f"./{i}.txt", 'a') as f:
        #             f.write("{} ".format(information.person.strength))
        #         break
    ...
    def act_play(self):
        ...
        """
        # TODO: 选手可自行做出其他处理
        information.record.update_rest(self._publicInfo)  # 更新手牌数量，并输出
        information.person.update_card(self._handCards)# 更新手牌
        print("当前手牌：{}".format(self._handCards))
